...
class JTreeReaderHelper {
  public:
    ~JTreeReaderHelper() {
      ...
      for (auto it = m_variable.begin(); it != m_variable.end(); ++it) {
        ...
        if (branchType == "ULong64_t") {
          ...
          if (it->second) delete static_cast<UChar_t *> (it->second);
    ...
    void clearStorage() {
      ...
      for (auto it = m_storage.begin(); it != m_storage.end(); ++it) {
        ...
        if (branchType == "Bool_t") {
          ...
          if (it->second) {
            delete static_cast<TTreeReaderValue<UChar_t> *> (it->second);
            it->second = 0;
          }
        ...
        } else if (branchType == "vector<UChar_t>") {
          if (it->second) {
            delete static_cast<TTreeReaderValue<vector<UChar_t>> *> (it->second);
            it->second = 0;
          }
        } else if (branchType == "vector<int>") {
          if (it->second) {
    ...
    void setBranches(TTreeReader & treeReader, map<string, string> branchNameToType) {
      for (auto it = branchNameToType.begin(); it != branchNameToType.end(); ++it) {
        ...
        if (!(treeReader.GetTree()->FindLeaf(branchName.c_str()))) { // Check if tree has branch
          ...
          m_storage[branchName] = new TTreeReaderValue<UChar_t> (treeReader, branchName.c_str());
          ...
          m_storage[branchName] = new TTreeReaderArray<UChar_t> (treeReader, branchName.c_str());
    ...
    UChar_t const & s_uchar(string const & member) const {
      #ifdef JDEBUG
      if (m_branchNameToType.find(member) == m_branchNameToType.end()) {
        cout<<"[Error] JTreeReaderHelper::s_uint(): No branch called "<<member<<endl;
        throw;
      }
      if (m_branchNameToType.at(member) != "UChar_t") {
        cout<<"[Error] JTreeReaderHelper::s_uint(): "<<member<<" type is "<<m_branchNameToType.at(member)<<endl;
        throw;
      }
      #endif
      return **(static_cast<TTreeReaderValue<UChar_t> *> (m_storage.at(member)));
    }
    ...
    TTreeReaderArray<UChar_t> const & v_uchar(string const & member) const {
      #ifdef JDEBUG
      if (m_branchNameToType.find(member) == m_branchNameToType.end()) {
        cout<<"[Error] JTreeReaderHelper::v_uchar(): No branch called "<<member<<endl;
        throw;
      }
      if (m_branchNameToType.at(member) != "vector<UChar_t>") {
        cout<<"[Error] JTreeReaderHelper::v_uchar(): "<<member<<" type is "<<m_branchNameToType.at(member)<<endl;
        throw;
      }
      #endif
      return *(static_cast<TTreeReaderArray<UChar_t> *> (m_storage.at(member)));
    }
...
class JTreeReader {
  public:
    ...
    // This will not work when using many files in a chain.
    TTreeReader::EEntryStatus SetEntry(Long64_t entry) {
      // TODO: Find which tree entry is in. Load correct tree. Load event.
      return m_currentReader->SetEntry(entry);
    }
